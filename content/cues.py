# -*- coding: utf-8 -*-
"""解題線索地圖的單一來源：題目線索／關鍵字 → 該用的知識點（跨單元）。

同時餵給兩處，改一次兩邊同步：
  ① 獨立「解題線索地圖」頁：build/build_cluemap.py（依 cat 分類彙整）。
  ② 各考點旁「🔑 解題線索」標籤：build_html._cues_html（依 unit+kp 反查）。

每條 cue：cat 分類、kw 線索關鍵字、hint 一句怎麼用、unit+kp 連到的考點。
...
CUES = [
    ...
    {"cat": "max", "kw": r"（正數）和固定／積固定求極值", "hint": r"**務必是正數 \(a,b>0\)！**算幾不等式 \(\dfrac{a+b}{2}\ge\sqrt{ab}\)：和固定 → 積在 \(a=b\) 時最大；積固定 → 和在 \(a=b\) 時最小", "unit": "numexpr", "kp": "kp4"},
    ...
    {"cat": "dist", "kw": r"點到平面距離", "hint": r"\(\dfrac{|ax_0+by_0+cz_0-d|}{\sqrt{a^2+b^2+c^2}}\)", "unit": "space", "kp": "kp4"},
    {"cat": "dist", "kw": r"兩點距離／中點", "hint": r"坐標差平方和開根號", "unit": "linecir", "kp": "kp2"},
    {"cat": "dist", "kw": r"\(|x-a|\) 的意義", "hint": r"\(x\) 到 \(a\) 的距離", "unit": "numexpr", "kp": "kp2"},

    # ⟂ 垂直・平行
    {"cat": "perp", "kw": r"向量垂直", "hint": r"內積 ＝ 0", "unit": "pvec", "kp": "kp3"},
    {"cat": "perp", "kw": r"兩直線垂直", "hint": r"斜率相乘 ＝ \(-1\)", "unit": "linecir", "kp": "kp1"},
    {"cat": "perp", "kw": r"向量平行（共線）", "hint": r"\(ad-bc=0\)", "unit": "pvec", "kp": "kp1"},
    ...
    {"cat": "angle", "kw": r"兩向量夾角", "hint": r"\(\cos\theta=\dfrac{\text{內積}}{\text{兩長度乘積}}\)", "unit": "pvec", "kp": "kp3"},
    ...
    {"cat": "angle", "kw": r"二面角", "hint": r"兩平面法向量的夾角", "unit": "space", "kp": "kp6"},

    # 🔺 面積・體積
    {"cat": "area", "kw": r"平行四邊形／三角形面積", "hint": r"\(|\)行列式 \(ad-bc|\)（三角形再 ÷2）", "unit": "pvec", "kp": "kp4"},
    ...
    {"cat": "area", "kw": r"圖形變換後的面積", "hint": r"原面積 \(\times|\det M|\)", "unit": "matrix", "kp": "kp5"},
