    ...
    "kps": [
        {
            ...
            "title": r"指數律與指數方程",
            "intro": r"指數的一切從 **指數律** 出發：同底相乘指數相加、次方再次方指數相乘。要會把根式與分數指數互換，並用「**化成同底**」解指數方程。",
            "points": [
                {"label": r"指數律", "lines": [
        ...
        },
        {
            ...
            "intro": r"對數是指數的反運算：\(\log_a N=x\Leftrightarrow a^x=N\)。熟練 **積、商、次方化成加減乘**，以及 **換底公式**，是本單元計算的核心。",
            "points": [
                {"label": r"定義", "lines": [
        ...
        },
        {
            ...
            "intro": r"常用對數（底 \(10\)）最會考「**一個大數有幾位**、**首位數字是什麼**」。關鍵：把數取 \(\log\)——**整數部分管位數、小數部分管首位**。",
            "points": [
                {"label": r"科學記號", "lines": [
        ...
        },
        {
            ...
            "title": r"指數與對數函數的圖形",
            "intro": r"指數函數 \(y=a^x\) 與對數函數 \(y=\log_a x\) **互為反函數**（圖形對稱於 \(y=x\)）。要掌握 **遞增／遞減、定義域、漸近線**，以及化簡時的「定義域陷阱」。",
            "points": [
        ...
        },
        {
            ...
            "intro": r"指數對數最實用的地方：**成長／衰退模型**（半衰期、複利）與 **數列結合**。核心口訣：**「取對數」把指數關係變一次、把等比變等差**。",
            "points": [
