                ...
                {"tag": "114 數A · 多選 11", "level": "★★★（難）", "core": "角平分線 ∩ 中線交點：以基底表示 ＋ 面積、內積",
                 ...
                 "solution": {"brief": r"(3)(4)(5)",
                              "steps": [
                                  ...
                                  r"**定 \(P\)**：\(\angle B\) 角平分線交 \(\overline{AC}\) 於 \(F\)，由角平分線定理 \(AF:FC=BA:BC=6:4=3:2\Rightarrow\overrightarrow{AF}=\dfrac35\overrightarrow{AC}\)。設 \(\overrightarrow{AP}=x\overrightarrow{AB}+y\overrightarrow{AC}\)。",
                                  r"\(P\) 在 \(\overline{CD}\)（\(D=\dfrac12\overrightarrow{AB}\)）上 → 係數和條件給 \(2x+y=1\)；\(P\) 在 \(\overline{BF}\) 上 → \(\dfrac{x}{1}+\dfrac{y}{3/5}=1\) 即 \(x+\dfrac53y=1\)。解得 \(x=\dfrac27,\,y=\dfrac37\)。",
