# -*- coding: utf-8 -*-
r"""從 exam/ 子站的題庫導出「核心概念 × 官方答對率」，產生 content/exam_rates.py。

資料流：
  ../exam/data.js（window.EXAM_DATA，200 題，每題有 rate 與 kaodian 核心概念連結）
      → 只取數 A（concept-map 是 108 課綱數 A）
      → 依 kaodian[].url 的 <檔名>#kpN 反查 units.py 的 slug
      → 同一 (slug, kp) 的題目取平均答對率
      → 只保留 n >= MIN_N 的組（樣本太小不足以代表核心概念難度）
...
# 樣本門檻：少於 3 題的核心概念不標數字。
# 單一題的難易受該題包裝影響極大，不等於整個核心概念的難易，標出來會誤導。
MIN_N = 3
...
def main():
    ...
    lines = [
        "# -*- coding: utf-8 -*-",
        '"""核心概念 × 官方答對率（大考中心）——由 build/gen_examrates.py 自動產生，請勿手改。',
        "",
        f"來源：exam/ 子站題庫（數 A {n_a} 題；非選擇題大考中心未列答對率，不計）。",
        f"多選題採「{metric_label}」（見 gen_examrates.MULTI_METRIC）。",
        f"只收錄樣本 >= {MIN_N} 題的核心概念：樣本太小時，單一題的包裝難易不等於核心概念難易。",
        "",
