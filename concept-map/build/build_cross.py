...
DETAIL = {
  ...
  <text x="124" y="104" font-size="10.5" fill="#1f3a93" font-style="italic">BD</text>
    ...
    "113|數A|11": {
        ...
        "opts": [r"擲出 \(a=b\) 的機率為 \(\tfrac13\)", r"此方程組無解的機率為 \(\tfrac1{12}\)",
                 r"此方程組有唯一解的機率為 \(\tfrac56\)", r"硬幣出現反面且此方程組有解的機率為 \(\tfrac12\)",
                 r"在硬幣出現反面且此方程組有解的條件下，\(x\) 值為正的機率為 \(\tfrac25\)"],
        "ans": r"**(2)(3)**",
  ...
  <text x="150" y="128" font-size="10" fill="#777" font-style="italic">平行六面體</text>
    ...
    "113|數A|8": {
        ...
        "opts": [r"\(T_n\) 皆為銳角三角形", r"\(T_2,T_3,T_4,\dots,T_{10}\) 的周長形成等差數列",
                 r"\(T_n\) 的面積隨 \(n\) 增大而增大", r"\(T_5\) 的三高依序形成等差數列",
