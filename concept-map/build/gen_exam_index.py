# -*- coding: utf-8 -*-
r"""從 exam/ 子站的題庫導出「考點 × 歷屆題清單」，產生 content/exam_qs.py。

這是 gen_examrates.py 的姊妹產線：同一份 kaodian 對應表，那支取「平均答對率」，
這支取「題目清單」，用來在考點卡上掛回 exam 的深連結。

資料流：
  ../exam/data.js（window.EXAM_DATA，200 題，每題有 kaodian 考點連結）
...
def rate_num(raw):
    ...
    卡片上顯示的那個數字，兩邊要一致（考點徽章的平均值才用全對率，見 gen_examrates）。
    非選擇題未列答對率 → None。
    """
...
def collect(questions):
    ...
    for key in out:
        ...
        for it in sorted(out[key], key=lambda x: (-x[0], x[1], x[2])):
            if it[:3] in seen:      # 同一題被同一考點掛兩次（理論上不會，防呆）
                continue
            seen.add(it[:3])
            uniq.append(it)
        out[key] = uniq
...
def main():
    ...
    print(f"  {years[0]}–{years[-1]}　數A {n_a} ＋ 數B {n_b} 題"
          f"　→ {len(data)} 個考點、{total_links} 條連結")

    # 覆蓋率報表：哪些考點掛不到題，一眼看得出來
    all_kps = []
    ...
    print(f"  覆蓋 {len(all_kps) - len(miss)}/{len(all_kps)} 個考點")
    if miss:
        print("  ⚠ 掛不到題的考點（exam 題庫中沒有對應）：")
    ...
    print("  題數最多的考點：" + "、".join(f"{s} {k}（{len(v)}）" for (s, k), v in top))
