...
| 檔案 | 說明 |
|------|------|
| `index.html` | **目錄首頁／網站入口**，列出所有單元 → 核心概念的樹狀連結 |
...
### 導覽與錨點
- 置頂工具列（`position:sticky; top:0`）：標題、`單元 ▾`（跨單元，用 `location.href`）、`跳至核心概念 ▾`（同頁，用 `location.hash`，跳完 reset `selectedIndex`）、全部顯示／隱藏。
- 每個核心概念卡片 `id="kp1"…"kp5"`；Part 2／3 `id="part2"`／`"part3"`；以 `scroll-margin-top:64px` 處理 sticky 遮擋。
...
- **Part 1 各核心概念**：① 核心概念標題 ② 這個核心概念在學什麼 ③ 重點與公式 ④ 常見誤解 ×→○（表） ⑤ 歷屆試題（題目＋簡答＋解題關鍵） ⑥ 解題策略
...
3. 在 `index.html` 加一張單元卡片（含核心概念子連結）。
