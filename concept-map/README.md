...
> **位置（2026-07 重整後）**：本子站位於 repo 的 `concept-map/`，正式網址
> `https://mathmap-kitty.github.io/web/concept-map/`；repo 根目錄是全站總入口 portal（另有 `exam/` 歷屆試題）。
> 本文所有「根目錄」一律指 `concept-map/` 這一層；建置請在 `concept-map/` 下執行 `python build/build.py`。
...
├── 115學測數學_矩陣.html      # 單元頁：矩陣（也是新單元的樣板範例）
...
## 本機預覽
直接用瀏覽器打開 `index.html` 即可（數學由本資料夾 `katex/` 離線渲染，**無須連網**）。

---

## 如何新增一個單元
1. 請老師用 `單元資料範本.docx` 填內容（或直接給文字）。
2. 複製 `115學測數學_矩陣.html`，替換成新單元內容（沿用所有 class 與 JS）。
3. 在 `index.html` 新增一張單元卡片（含各核心概念子連結）。
4. 在**每個**單元頁的「單元 ▾」下拉，加入新單元選項。

**內容轉換約定**：範本中用 **【 】** 框的＝做成「填空」；「常見誤解的正解」「歷屆／練習的簡答、詳解」＝自動做成「顯示解答」按鈕。

每個單元的結構：Part 1 各核心概念（這個核心概念在學什麼／重點與公式／常見誤解 ×→○／歷屆試題／解題策略）、Part 2 模擬實戰、Part 3 考前速查。
...
- 數學：**KaTeX 0.16.11（本地 `katex/`，含 contrib/auto-render，2026-07 起不走 CDN）**；行內 `\(...\)`、獨立行 `\[...\]`。引用路徑由各 build 腳本的 `KATEX = "katex"` 常數控制。
...
### 導覽與配色
- 置頂工具列（sticky）：標題、單元 ▾、跳至核心概念 ▾、全部顯示／隱藏。
- 核心概念卡片 `id="kp1"…"kp5"`，Part 2/3 `id="part2"/"part3"`，`scroll-margin-top:64px` 處理遮擋。
...
- ~~KaTeX 改**本地打包**以支援完全離線。~~（2026-07 已完成：`katex/` 本地包）
