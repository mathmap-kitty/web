...
UNIT = {
    ...
    "part0": {
        ...
        # B1 直覺挑戰（老師定案 L2）：垂直＝負倒數，倒數/變號只做一半都錯
        "challenge": {
            "q": r"斜率 \(3\) 的直線，跟哪個斜率的直線互相垂直？",
            "options": [r"\(\dfrac13\)", r"\(-3\)", r"\(-\dfrac13\)", r"\(3\)"],
            "answer": 2,
            "reveal": r"答 \(\dfrac13\) 的人記成「倒數」、答 \(-3\) 的人記成「變號」——垂直要 **負倒數**，兩件事都要做：\(m_1m_2=-1\)。",
            "kp": "kp1",
        },
        ...
        # 方案 B 第二個單元。四格沿用 ChatGPT 試作版的分法（方向／方程／距離／區域）。
        "axes": {
            "intro": (r"直線與圓不是公式表，是先判斷題目在講**方向**、**方程**、**距離**還是**區域**。"
                      r"實際考題**多半是兩格的組合**（像「距離 ▸ 方程」＝先用 \(d\) 與 \(r\) 判位置，再列式算弦長）"
                      r"——**分對入口那一格，第二格通常自己會浮出來。**"),
            "items": [
                {"key": "方向", "desc": r"斜率、平行、垂直、傾角",
                 "hint": r"先抓斜率（或方向向量），再補上一個點就成直線"},
                {"key": "方程", "desc": r"點斜式、一般式、圓標準式",
                 "hint": r"把條件固定成可算的式子；圓題先配方讀出圓心與半徑"},
                {"key": "距離", "desc": r"點到直線、兩點距離、\(d\) 與 \(r\) 比大小",
                 "hint": r"把幾何關係變成一個數字，再跟半徑或已知長度比"},
                {"key": "區域", "desc": r"不等式、半平面、可行域",
                 "hint": r"先畫邊界，再用測試點決定取哪一側"},
            ],
        },

        "entrydiag": {
            "heading": "先做入口診斷：只選抽屜，不要算",
            "note": (r"每題只問「**第一步**從哪裡切入」，不是問整題只用那一格。"
                     r"答錯沒關係——**分不出入口才是真正卡住的地方**。"),
            "cards": [
                {"q": r"求過點 \((2,1)\) 且與直線 \(3x-4y+5=0\) 平行的直線方程式。",
                 "a": "方向", "why": r"「平行」講的是方向——先確定斜率一樣，再用那個點把位置補上。", "kp": "kp1"},
                {"q": r"直線 \(y=2x+1\) 與 \(y=kx-3\) 互相垂直，求 \(k\)。",
                 "a": "方向", "why": r"垂直是兩條線的方向關係：斜率相乘 \(=-1\)，不必畫圖也不必解交點。", "kp": "kp1"},
                {"q": r"已知圓心 \((2,-1)\)、半徑 \(3\)，寫出圓的方程式。",
                 "a": "方程", "why": r"圓心與半徑都給了，直接套標準式 \((x-h)^2+(y-k)^2=r^2\) 把它固定成式子。", "kp": "kp3"},
                {"q": r"將 \(x^2+y^2-6x+4y-3=0\) 化為標準式，並求圓心與半徑。",
                 "a": "方程", "why": r"一般式看不出圓心，先配方變成標準式——這是在整理式子，不是在算距離。", "kp": "kp3"},
                {"q": r"求點 \((3,4)\) 到直線 \(3x+4y-10=0\) 的距離。",
                 "a": "距離", "why": r"直接問距離，套點到直線距離公式即可（分母是 \(\sqrt{a^2+b^2}\)）。", "kp": "kp1"},
                {"q": r"判斷直線 \(x+y=5\) 與圓 \(x^2+y^2=8\) 是相離、相切還是相交。",
                 "a": "距離", "why": r"**不要聯立**：算圓心到直線的距離 \(d\)，跟半徑 \(r\) 比大小就有答案。", "kp": "kp4"},
                {"q": r"在 \(x+y\le4\)、\(x\ge0\)、\(y\ge0\) 所圍的區域中，求 \(2x+3y\) 的最大值。",
                 "a": "區域", "why": r"先畫出可行域，線性規劃的極值一定發生在**頂點**，只要檢查頂點。", "kp": "kp5"},
                {"q": r"判斷 \(2x-y+1>0\) 表示直線 \(2x-y+1=0\) 的哪一側。",
                 "a": "區域", "why": r"符號不會直接告訴你哪一側——先畫邊界，再代一個測試點（例如原點）驗證。", "kp": "kp5"},
            ],
        },
    },
    ...
    "kps": [
        {
            "num": "核心概念 1", "id": "kp1", "nav": "直線方程式與斜率", "freq": "★★★ 必考",
            # 這個核心概念同時含「點到直線距離」，其實跨方向／方程／距離三格。
            # 老師 2026-08-06 定案：取核心概念名稱的主軸「方向 ▸ 方程」，不改成「方向 ▸ 距離」。
            "axis": ["方向", "方程"],
        ...
        {
            "num": "核心概念 2", "id": "kp2", "nav": "兩點距離與三角形", "freq": "★★☆",
            "axis": ["距離", "方程"],
            "title": r"兩點距離、中點與三角形",
        ...
        {
            "num": "核心概念 3", "id": "kp3", "nav": "圓方程式", "freq": "★★☆",
            # 取「方程」是因為這裡的主活動是標準式／一般式互換與配方；
            # 次格取「距離」——圓的本質就是「到圓心距離＝r」，判點在圓內外也是比距離。
            "axis": ["方程", "距離"],
            "title": r"圓方程式與點的位置",
            "intro": r"圓由「**圓心 ＋ 半徑**」決定。要會在標準式與一般式間轉換、由一般式求圓心半徑，並用「**代入比大小**」判斷點在圓內、圓上或圓外。",
            "prereq": [
                r"**配方**（一般式 ↔ 標準式）",
                r"**兩點距離**（半徑、點到圓心）（核心概念 2）",
                r"圓「**圓心 ＋ 半徑**」的概念",
            ],
            "points": [
                {"label": r"標準式", "lines": [
        ...
        {
            "num": "核心概念 4", "id": "kp4", "nav": "直線與圓的位置", "freq": "★★★ 必考",
            "axis": ["距離", "方程"],
            ...
            "prereq": [
                ...
                r"**圓心、半徑**（核心概念 3）",
                r"**畢氏定理**（弦長、切線長都靠它）",
                r"圓的參數式 \((\cos\theta,\sin\theta)\) 與 **疊合**（三角單元 核心概念 1、4）——\(108\) 單1 會用",
            ],
            "points": [
        ...
        {
            "num": "核心概念 5", "id": "kp5", "nav": "平面區域與線性規劃", "freq": "★★☆",
            "axis": ["區域", "方程"],
            "title": r"平面區域與線性規劃",
