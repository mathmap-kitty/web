...
UNIT = {
    ...
    "title": "空間向量",
    "exam_tag": "115 學測",
    "hero_sub": "Part 1 六大核心概念 ｜ Part 2 模擬實戰 ｜ Part 3 考前速查",
    "hero_sub2": "每個核心概念皆含：重點與公式 · 常見誤解 · 歷屆試題 · 解題策略",
    "part1_label": "六大核心概念",
    "foot": "115 學測數學 · 空間向量 · 學測數學重點整理",

    "part0": {
        "opener": {"svg": soil_maps.SLIDES["space"], "hero": True,
    ...
    "mixed": [
        {
            "src": "113 學測數A · 第 18–20 題組（大考中心原卷 · 官方評分原則）",
            "context": r"坐標空間中，\(O\) 為原點，\(E\) 為平面 \(x-z=4\)。",
            "items": [
                {"tag": "第 18 題 · 單選 · 3 分",
                 "q": r"若原點 \(O\) 在平面 \(E\) 上的投影點為 \(Q\)，且 \(\overrightarrow{OQ}\) 與 \((1,0,0)\) 的夾角為 \(\theta\)，則 \(\cos\theta=\)？",
                 "options": [r"\(-\dfrac{\sqrt2}{2}\)", r"\(-\dfrac12\)", r"\(\dfrac12\)", r"\(\dfrac{\sqrt2}{2}\)", r"\(\dfrac{\sqrt3}{2}\)"],
                 "answer": r"**(4) \(\dfrac{\sqrt2}{2}\)**。\(E\) 的法向量 \(\vec n=(1,0,-1)\)，\(O\) 沿 \(\vec n\) 投影：\(Q=(t,0,-t)\) 代入 \(x-z=4\) 得 \(2t=4\)、\(Q=(2,0,-2)\)。\(\overrightarrow{OQ}=(2,0,-2)\)、\(|\overrightarrow{OQ}|=2\sqrt2\)，故 \(\cos\theta=\dfrac{2}{2\sqrt2}=\dfrac{\sqrt2}{2}\)。"},
                {"tag": "第 19 題 · 非選 · 4 分",
                 "q": r"已知空間中一點 \(P(a,b,c)\) 滿足 \(\overrightarrow{OP}\) 與 \((1,0,0)\) 的夾角 \(\le\dfrac{\pi}{6}\)。試說明 \(a^2\ge3(b^2+c^2)\)。",
                 "answer": r"夾角 \(\le\dfrac\pi6\Rightarrow\cos(\text{夾角})=\dfrac{a}{\sqrt{a^2+b^2+c^2}}\ge\cos\dfrac\pi6=\dfrac{\sqrt3}{2}\)（此時 \(a>0\)）。兩邊平方：\(\dfrac{a^2}{a^2+b^2+c^2}\ge\dfrac34\)，即 \(4a^2\ge3(a^2+b^2+c^2)\)，整理得 \(a^2\ge3(b^2+c^2)\)。",
                 "rubric": {"full": [r"寫出 \(\dfrac{a}{\sqrt{a^2+b^2+c^2}}\ge\dfrac{\sqrt3}{2}\) 並正確推得 \(a^2\ge3(b^2+c^2)\)，過程完整。"],
                            "partial": r"上述推導過程部分正確。",
                            "zero": r"未作答，或未符合部分給分原則。"}},
                {"tag": "第 20 題 · 非選 · 8 分",
                 "q": r"承上，已知 \(P\) 在平面 \(E\) 上且 \(b=0\)。試求 \(c\) 的最大可能範圍，並求 \(\overline{OP}\) 的最小可能長度。",
                 "answer": r"\(b=0\)、\(P\) 在 \(E\)：\(a-c=4\Rightarrow a=c+4\)。由第 19 題（\(b=0\)）\(a^2\ge3c^2\)：\((c+4)^2\ge3c^2\Rightarrow c^2-4c-8\le0\Rightarrow 2-2\sqrt3\le c\le2+2\sqrt3\)。又 \(\overline{OP}^2=a^2+c^2=(c+4)^2+c^2=2c^2+8c+16\)，其頂點 \(c=-2\) 在此區間左外、於區間上遞增，最小值在 \(c=2-2\sqrt3\)：\(\overline{OP}^2=64-32\sqrt3=16(\sqrt3-1)^2\)，故最小長度 \(\overline{OP}=4(\sqrt3-1)\)。",
                 "rubric": {"full": [r"求得 \(c\) 的範圍 \(2-2\sqrt3\le c\le2+2\sqrt3\)。",
                                     r"求得 \(\overline{OP}\) 的最小長度 \(4(\sqrt3-1)\)。"],
                            "partial": r"僅求得 \(c\) 的範圍，或最小長度的過程部分正確。",
                            "zero": r"未作答，或未符合部分給分原則。"}},
            ],
        },
        {
            "src": "115 學測數A · 第 18–20 題組（大考中心原卷 · 官方評分原則）",
            "context": r"坐標空間中有一平行六面體 \(PQRS\text{-}ABCD\)。已知 \(\overrightarrow{AB}\times\overrightarrow{AD}=(-5,5,5)\)、\(\overrightarrow{AD}\times\overrightarrow{AP}=(-2,0,-4)\)、\(\overrightarrow{AP}\times\overrightarrow{AB}=(6,-10,-8)\)，且 \(|\overrightarrow{AP}|=6\)。",
            "items": [
                {"tag": "第 18 題 · 單選 · 3 分",
                 "q": r"試問平行四邊形 \(ABCD\) 的面積為何？",
                 "options": [r"\(2\sqrt5\)", r"\(5\sqrt2\)", r"\(5\sqrt3\)", r"\(6\sqrt3\)", r"\(10\sqrt2\)"],
                 "answer": r"**(3) \(5\sqrt3\)**。平行四邊形 \(ABCD\) 以 \(\overrightarrow{AB},\overrightarrow{AD}\) 為鄰邊，面積 \(=|\overrightarrow{AB}\times\overrightarrow{AD}|=|(-5,5,5)|=\sqrt{75}=5\sqrt3\)。"},
                {"tag": "第 19 題 · 非選 · 4 分",
                 "q": r"設 \(B\) 點坐標為 \((1,2,0)\)，試求平面 \(ABCD\) 的平面方程式。",
                 "answer": r"平面 \(ABCD\) 的法向量平行於 \(\overrightarrow{AB}\times\overrightarrow{AD}=(-5,5,5)\)，即平行於 \((1,-1,-1)\)。過 \(B(1,2,0)\)：\((x-1)-(y-2)-(z-0)=0\)，整理得 \(x-y-z+1=0\)。",
                 "rubric": {"full": [r"算出平面 \(ABCD\) 的法向量平行於 \((-5,5,5)\)。",
                                     r"代入 \(B(1,2,0)\) 得平面方程式 \(x-y-z+1=0\)。"],
                            "partial": r"僅正確指出法向量平行 \((-5,5,5)\)，未完整導出方程式。",
                            "zero": r"未作答，或未符合部分給分原則。"}},
                {"tag": "第 20 題 · 非選 · 8 分",
                 "q": r"試求平行六面體的體積，並求平行六面體上（含邊界）距點 \(A\) 的最長距離。",
                 "answer": r"因 \(\overrightarrow{AP}\perp(\overrightarrow{AD}\times\overrightarrow{AP})\) 且 \(\overrightarrow{AP}\perp(\overrightarrow{AP}\times\overrightarrow{AB})\)，故 \(\overrightarrow{AP}\parallel(\overrightarrow{AD}\times\overrightarrow{AP})\times(\overrightarrow{AP}\times\overrightarrow{AB})=(-2,0,-4)\times(6,-10,-8)=-20(2,2,-1)\)；又 \(|\overrightarrow{AP}|=6\)，得 \(\overrightarrow{AP}=(4,4,-2)\)。**體積** \(=|\overrightarrow{AP}\cdot(\overrightarrow{AB}\times\overrightarrow{AD})|=|(4,4,-2)\cdot(-5,5,5)|=|-10|=10\)。再解得 \(\overrightarrow{AB}=(1,-1,2)\)、\(\overrightarrow{AD}=(2,3,-1)\)，故 \(\overrightarrow{AC}=(3,2,1)\)、\(\overrightarrow{AS}=(6,7,-3)\)、\(\overrightarrow{AQ}=(5,3,0)\)、\(\overrightarrow{AR}=(7,6,-1)\)；比較各頂點到 \(A\) 的距離，以 \(|\overrightarrow{AS}|=\sqrt{94}\) 最大，故**最長距離 \(=\sqrt{94}\)**。",
                 "rubric": {"full": [r"正確計算得到 \(\overrightarrow{AP}\)。",
                                     r"正確計算得平行六面體體積 \(=10\)。",
                                     r"正確計算得到 \(\overrightarrow{AB},\overrightarrow{AD}\)。",
                                     r"由各頂點判斷距 \(A\) 的最長距離為 \(\sqrt{94}\)。"],
                            "partial": r"上述四項僅部分正確（例如只求得 \(\overrightarrow{AP}\)、或只求到體積）。",
                            "zero": r"未作答，或未符合部分給分原則。"}},
            ],
        },
    ],
    ...
    "kps": [
        {
            ...
            "worked": {
                "q": r"求 \(A(2,3,1)\)、\(B(4,3,5)\) 的距離與中點。",
                "steps": [
                    {"do": r"距離 \(\overline{AB}=\sqrt{(4-2)^2+(3-3)^2+(5-1)^2}=\sqrt{4+0+16}=\sqrt{20}=2\sqrt5\)。",
                     "why": r"空間距離 ＝ 三個坐標差的平方和開根號（比平面多一個 \(z\) 項）。"},
                    {"do": r"中點 \(M=\left(\dfrac{2+4}{2},\,\dfrac{3+3}{2},\,\dfrac{1+5}{2}\right)=(3,3,3)\)。",
                     "why": r"中點 ＝ 兩端點對應坐標**相加除以 \(2\)**。"},
                ],
            },
            "questions": [
        ...
        },
        {
            ...
            "worked": {
                "q": r"\(\vec u=(1,1,0)\)、\(\vec v=(1,0,1)\)，求 \(\vec u\cdot\vec v\) 與兩向量夾角 \(\theta\)。",
                "steps": [
                    {"do": r"內積 \(\vec u\cdot\vec v=1\cdot1+1\cdot0+0\cdot1=1\)。",
                     "why": r"坐標式內積 ＝ 對應坐標相乘再相加。"},
                    {"do": r"\(|\vec u|=|\vec v|=\sqrt{1+1+0}=\sqrt2\)，\(\cos\theta=\dfrac{1}{\sqrt2\cdot\sqrt2}=\dfrac12\)，故 \(\theta=60°\)。",
                     "why": r"\(\cos\theta=\dfrac{內積}{兩長度相乘}\)；\(\cos\theta=\dfrac12\) 對應 \(60°\)。"},
                ],
            },
            "questions": [
            ...
            "strategy": [
                ...
                r"「對一堆頂點兩兩取內積再平均」型期望值（如本核心概念 \(112\) 單6），善用 \(\sum_{i<j}\vec{u_i}\cdot\vec{u_j}=\frac12\big(|\sum\vec{u_i}|^2-\sum|\vec{u_i}|^2\big)\)。",
            ],
        },
        {
            "num": "核心概念 3", "id": "kp3", "nav": "外積與體積", "freq": "★★★ 必考",
            ...
            "worked": {
                "q": r"\(\vec u=(2,0,0)\)、\(\vec v=(0,3,0)\)，求 \(\vec u\times\vec v\)，並求以 \(\vec u,\vec v\) 為鄰邊的平行四邊形面積。",
                "steps": [
                    {"do": r"\(\vec u\times\vec v=(0\cdot0-0\cdot3,\;0\cdot0-2\cdot0,\;2\cdot3-0\cdot0)=(0,0,6)\)。",
                     "why": r"套外積坐標公式；算出的 \((0,0,6)\) 指向 \(z\) 軸，**同時垂直** 躺在 \(xy\) 平面的 \(\vec u,\vec v\)。"},
                    {"do": r"平行四邊形面積 \(=|\vec u\times\vec v|=\sqrt{0^2+0^2+6^2}=6\)。",
                     "why": r"外積的長度就等於以兩向量為鄰邊的平行四邊形面積。"},
                ],
            },
            "questions": [
                ...
                {"tag": "112 數A · 單選 5", "level": "★★★（難）", "core": "行列式＝純量三重積：向量在平面上 → 法向量方向",
                 "body": r"坐標空間中 \(P、Q、R\) 為平面 \(2x-3y+5z=\sqrt7\) 上不共線三點。令 \(\overrightarrow{PQ}=(a_1,b_1,c_1)、\overrightarrow{PR}=(a_2,b_2,c_2)\)。下列行列式（第一列分別為所給向量、第二三列為 \(\overrightarrow{PQ},\overrightarrow{PR}\)）何者絕對值最大？",
                 "options": [
                     r"第一列 \((-1,1,1)\)",
                     r"第一列 \((1,-1,1)\)",
                     r"第一列 \((1,1,-1)\)",
                 ...
                 "solution": {"brief": r"(2)",
                              "steps": [
                                  r"行列式 \(\begin{vmatrix}\vec v\\ \overrightarrow{PQ}\\ \overrightarrow{PR}\end{vmatrix}=\vec v\cdot(\overrightarrow{PQ}\times\overrightarrow{PR})\)（純量三重積）。",
                                  r"\(\overrightarrow{PQ},\overrightarrow{PR}\) 都在平面上 → \(\overrightarrow{PQ}\times\overrightarrow{PR}\) 平行平面法向量 \((2,-3,5)\)。故 \(|\text{行列式}|\propto|\vec v\cdot(2,-3,5)|\)。",
                                  r"各選項 \(\vec v\cdot(2,-3,5)\)：(1)\(0\)、(2)\(2+3+5=10\)、(3)\(-6\)、(4)\(6\)、(5)\(-4\)。絕對值最大為 (2)。"]}},
                {"tag": "111 數A · 選填 17", "level": "★★★（難）", "core": "平行六面體最大體積：底面積（外積）× 到底面距離",
                 "body": r"坐標空間中一平行六面體，某底面的其中三頂點為 \((-1,2,1)、(-4,1,3)、(2,0,-3)\)，另一面之一頂點在 \(xy\) 平面上且與原點距離為 \(1\)。求此類平行六面體的最大體積。",
                 "solution": {"brief": r"\(21\)",
                              "steps": [
                                  r"底面向量 \(\vec u=(-3,-1,2)、\vec w=(3,-2,-4)\)，底面積 \(=|\vec u\times\vec w|=|(8,-6,9)|=\sqrt{181}\)，底面 \(8x-6y+9z+11=0\)。",
                                  r"頂點 \(V=(x,y,0)\) 在單位圓 \(x^2+y^2=1\) 上；體積 \(=\) 底面積 \(\times\) 距離 \(=\sqrt{181}\cdot\dfrac{|8x-6y+11|}{\sqrt{181}}=|8x-6y+11|\)。",
                                  r"\(x^2+y^2=1\) 上 \(\max(8x-6y)=\sqrt{8^2+6^2}=10\)（柯西不等式，見平面向量單元 核心概念 3；或設 \((x,y)=(\cos\theta,\sin\theta)\) 疊合），故最大體積 \(=10+11=21\)。"]}},
            ],
            "selfcheck": {
        ...
        {
            "num": "核心概念 4", "id": "kp4", "nav": "平面方程式與距離", "freq": "★★★ 必考",
            "title": r"平面方程式、投影與點到平面距離",
            "intro": r"平面用「**法向量 ＋ 一點**」就能決定。本核心概念是 ★★★ 最大重鎮：求平面方程式、點到平面距離、投影點。武器仍是 **外積**（求法向量）＋ **內積**（判平面上、求夾角）。",
            "prereq": [
                r"**外積**求法向量、**內積**判垂直（核心概念 2、3）",
                r"**向量** \(\overrightarrow{AB}=B-A\) 與長度",
                r"平面方程式 \(ax+by+cz=d\) 的長相",
            ],
            "points": [
                {"label": r"平面方程式", "lines": [
            ...
            "worked": {
                "q": r"求點 \(P(1,2,2)\) 到平面 \(2x+y-2z=1\) 的距離。",
                "steps": [
                    {"do": r"法向量 \(\vec n=(2,1,-2)\)，長度 \(\sqrt{2^2+1^2+(-2)^2}=3\)。",
                     "why": r"\(ax+by+cz=d\) 的法向量直接讀係數 \((a,b,c)\)。"},
                    {"do": r"距離 \(=\dfrac{|2\cdot1+1\cdot2-2\cdot2-1|}{3}=\dfrac{|-1|}{3}=\dfrac13\)。",
                     "why": r"點到平面距離 ＝ \(\dfrac{|ax_0+by_0+cz_0-d|}{\sqrt{a^2+b^2+c^2}}\)，分子記得取絕對值。"},
                ],
            },
            "questions": [
                ...
                {"tag": "111 數A · 選填 16", "level": "★★★（難）", "core": "平面上直線的投影：作垂直於原平面的平面",
                 "body": r"坐標空間中，平面 \(x-y+2z=3\) 上有兩相異直線 \(L:\dfrac x2-1=y+1=-2z\) 與 \(L'\)。已知 \(L\) 也在另一平面 \(E\) 上，且 \(L'\) 在 \(E\) 的投影與 \(L\) 重合。求 \(E\) 的方程式（\(x\) 係數為 \(1\)）。",
                 "solution": {"brief": r"\(x-3y-2z=5\)",
                              "steps": [
                                  r"\(L\)：化為 \(\dfrac{x-2}{2}=\dfrac{y+1}{1}=\dfrac{z}{-1/2}\)，過 \((2,-1,0)\)、方向 \(\vec d=(4,2,-1)\)（在平面 \(\pi:x-y+2z=3\) 內）。",
                                  r"\(L'\) 也在 \(\pi\) 內；欲使 \(\pi\) 內任一直線 \(L'\) 投影到 \(L\)，需 \(E\perp\pi\) 且 \(E\cap\pi=L\)（此時整個 \(\pi\) 沿 \(E\) 的法向壓到直線 \(L\) 上）。",
                                  r"故 \(E\) 的法向量 \(\vec n_E\perp\vec d\) 且 \(\perp\vec n_\pi=(1,-1,2)\)：\(\vec n_E\parallel\vec d\times\vec n_\pi=(4,2,-1)\times(1,-1,2)=(3,-9,-6)\parallel(1,-3,-2)\)。",
                                  r"過 \((2,-1,0)\)：\(x-3y-2z=2-3(-1)-0=5\)，故 \(E:x-3y-2z=5\)。"]}},
                {"tag": "114 數A · 選填 14", "level": "★★★（難）", "core": "點到平面距離公式：解聯立求坐標",
                 "body": r"坐標空間中點 \(A=(a,b,c)\)，\(a,b,c\) 皆為小於 \(0\) 的實數。已知 \(A\) 與三平面 \(E_1:4y+3z=2、E_2:3y+4z=-5、E_3:x+2y+2z=-2\) 的距離都是 \(6\)。求 \(a+b+c\)。",
                 "solution": {"brief": r"\(-11\)",
                              "steps": [
                                  r"\(E_1\)：\(\dfrac{|4b+3c-2|}{5}=6\)。因 \(b,c<0\Rightarrow4b+3c-2<0\)，取 \(4b+3c-2=-30\Rightarrow4b+3c=-28\)。",
                                  r"\(E_2\)：\(\dfrac{|3b+4c+5|}{5}=6\)，同理 \(3b+4c+5=-30\Rightarrow3b+4c=-35\)。解得 \(b=-1,\ c=-8\)。",
                                  r"\(E_3\)：\(\dfrac{|a+2b+2c+2|}{3}=6\Rightarrow|a-16|=18\)，\(a<0\Rightarrow a=-2\)。",
                                  r"故 \(a+b+c=-2-1-8=-11\)。"]}},
            ],
            "selfcheck": {
        ...
        },
        {
            ...
            "worked": {
                "q": r"直線 \(L\) 過 \(A(1,0,0)\)、方向 \(\vec d=(1,1,1)\)。寫出 \(L\) 的參數式，並判斷 \(L\) 與平面 \(E:x+y+z=5\) 的關係。",
                "steps": [
                    {"do": r"參數式 \((x,y,z)=(1+t,\,t,\,t)\)，\(t\in\mathbb{R}\)。",
                     "why": r"直線 ＝ 一點 ＋ \(t\,\times\) 方向向量。"},
                    {"do": r"方向 \(\vec d=(1,1,1)\) 與平面法向量 \(\vec n=(1,1,1)\) 平行 \(\Rightarrow L\perp E\)。",
                     "why": r"方向向量平行於法向量時，直線垂直於平面。"},
                ],
            },
            "questions": [
                ...
                {"tag": "112 數A · 選填 17", "level": "★★★（難）", "core": "公垂線：兩歪斜線距離 ＋ 畢氏合成",
                 "body": r"坐標空間中兩不相交直線 \(L_1:(x,y,z)=(1+t,\,1-t,\,2+t)\)、\(L_2:(x,y,z)=(2+2s,\,5+s,\,6-s)\)。另一直線 \(L_3\) 與 \(L_1、L_2\) 皆相交且垂直。\(P、Q\) 兩點分別在 \(L_1、L_2\) 上且與 \(L_3\) 的距離皆為 \(3\)，求 \(\overline{PQ}\)（最簡根式）。",
                 "solution": {"brief": r"\(5\sqrt2\)",
                              "steps": [
                                  r"\(L_3\) 為公垂線，交 \(L_1\) 於 \(M\)、交 \(L_2\) 於 \(N\)。因 \(L_3\perp L_1\)，\(P\) 在 \(L_1\) 上，故 \(\text{dist}(P,L_3)=\overline{PM}=3\)；同理 \(\overline{QN}=3\)。",
                                  r"方向 \(\vec{d_1}=(1,-1,1)、\vec{d_2}=(2,1,-1)\)，\(\vec{d_1}\cdot\vec{d_2}=0\)（互相垂直）；且 \(\overrightarrow{MN}\perp\vec{d_1},\vec{d_2}\)。",
                                  r"\(\overrightarrow{PQ}=\overrightarrow{MN}\pm3\hat{d_2}\mp3\hat{d_1}\)，三者兩兩垂直 \(\Rightarrow\overline{PQ}^2=\overline{MN}^2+3^2+3^2\)。",
                                  r"\(\overline{MN}=\dfrac{|\overrightarrow{A_1A_2}\cdot(\vec{d_1}\times\vec{d_2})|}{|\vec{d_1}\times\vec{d_2}|}=\dfrac{|(1,4,4)\cdot(0,3,3)|}{|(0,3,3)|}=\dfrac{24}{3\sqrt2}=4\sqrt2\)。",
                                  r"\(\overline{PQ}^2=(4\sqrt2)^2+18=32+18=50\Rightarrow\overline{PQ}=5\sqrt2\)。"]}},
            ],
            "selfcheck": {
            ...
            "strategy": [
                ...
                r"「使三角形為正三角形／等腰」的 **點軌跡**（對應本核心概念 \(107\) 單1）：拆成「到兩點等距 → 中垂面」「到一點等距 → 球面」，再求兩者交集。",
            ],
        },
        {
            "num": "核心概念 6", "id": "kp6", "nav": "二面角與立體", "freq": "★★☆",
            "title": r"二面角與立體（摺疊問題）",
            ...
            "worked": {
                "q": r"兩平面的法向量分別為 \(\vec{n_1}=(1,1,0)\)、\(\vec{n_2}=(0,1,1)\)，求兩平面夾角 \(\theta\) 的 \(\cos\theta\)。",
                "steps": [
                    {"do": r"兩平面夾角 ＝ 兩法向量夾角：\(\vec{n_1}\cdot\vec{n_2}=1\cdot0+1\cdot1+0\cdot1=1\)。",
                     "why": r"二面角（兩平面夾角）可由兩面的法向量夾角求得。"},
                    {"do": r"\(|\vec{n_1}|=|\vec{n_2}|=\sqrt2\)，\(\cos\theta=\dfrac{1}{\sqrt2\cdot\sqrt2}=\dfrac12\)（即 \(60°\)）。",
                     "why": r"\(\cos\theta=\dfrac{內積}{兩長度相乘}\)。"},
                ],
            },
            "questions": [
                ...
                {"tag": "111 數A · 多選 11", "level": "★★★（難）", "core": "三角柱積木：坐標化算二面角、角度比較",
                 "body": r"積木中 \(\triangle ABC\) 為直角三角形，\(\angle ACB=90^\circ\)，\(\overline{AC}=5、\overline{BC}=6\)，且 \(ADEB\) 與 \(ADFC\) 皆為矩形（\(\overline{AD}\) 垂直底面 \(\triangle ABC\)）。試選出正確的選項。",
                 "options": [
                     r"沿平面 \(ACE\) 切下可切得兩個四面體",
                     r"平面 \(ADEB\) 與 \(ADFC\) 所夾銳角大於 \(45^\circ\)",
                     r"\(\angle CEB<\angle AEB\)",
                     r"\(\tan\angle AEC<\sin\angle CEB\)",
                     r"\(\angle CEB<\angle AEC\)"],
                 "solution": {"brief": r"(2)(3)(4)",
                              "steps": [
                                  r"設 \(C=(0,0,0)、A=(5,0,0)、B=(0,6,0)\)，\(\overline{AD}\perp\) 底面故 \(D=(5,0,h)、E=(0,6,h)、F=(0,0,h)\)（\(h=\overline{AD}\)）。",
                                  r"(1)✗ 平面 \(ACE\) 把柱體分成四面體 \(ABCE\) 與另一個五頂點體 \(ACEFD\)，並非兩個四面體。",
                                  r"(2)○ 兩矩形沿公共稜 \(\overline{AD}\) 的夾角 ＝ \(\angle BAC\)，\(\tan\angle BAC=\dfrac{BC}{AC}=\dfrac65>1\Rightarrow\angle BAC\approx50^\circ>45^\circ\)。",
                                  r"(3)○ \(\cos\angle CEB=\dfrac{h}{\sqrt{36+h^2}}>\dfrac{h}{\sqrt{61+h^2}}=\cos\angle AEB\Rightarrow\angle CEB<\angle AEB\)。",
                                  r"(4)○ 算得 \(\tan\angle AEC=\dfrac{5}{\sqrt{36+h^2}}<\dfrac{6}{\sqrt{36+h^2}}=\sin\angle CEB\)。",
                                  r"(5)✗ 比較 \(\cos\)：\(h^2(61+h^2)<(36+h^2)^2\Rightarrow\cos\angle CEB<\cos\angle AEC\Rightarrow\angle CEB>\angle AEC\)。故選 (2)(3)(4)。"]}},
            ],
            "selfcheck": {
    ...
    "part2": {
        "count": "10", "note": "難度貼近學測，涵蓋六大核心概念 · 建議自行限時 40 分鐘作答後再對照詳解",
        "groups": [
