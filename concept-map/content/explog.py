...
UNIT = {
    ...
    "kps": [
        {
            "num": "核心概念 1", "id": "kp1", "nav": "指數律與指數方程", "freq": "★★☆",
            "title": r"指數律與指數方程",
            "intro": r"**指數就是「連乘」的簡記**：\(2^3\) 就是 \(2\) 連乘 \(3\) 次。抓住這一點，指數律全都能「**數出來**」——\(2^3\times2^2=(2\cdot2\cdot2)\times(2\cdot2)=2^5\)，數一數 \(2\) 的個數，就知道同底相乘指數要**相加**；\((2^3)^2=2^3\times2^3=2^6\)，所以「次方再次方」是**相乘**。本核心概念就是把這些規則練熟，再用「**化成同底**」來解指數方程。",
            "prereq": [
            ...
            "points": [
                ...
                {"label": r"分數指數（根式）", "lines": [
                    ...
                    r"**連續開 \(n\) 次平方根**（開完的結果再開、共開 \(n\) 次）＝ 取 \(\left(\dfrac12\right)^n=\dfrac{1}{2^n}\) 次方（[[舉例||一次平方根是 \(a^{1/2}\)；連開 \(3\) 次：\(\sqrt{\sqrt{\sqrt a}}=a^{1/2}\to a^{1/4}\to a^{1/8}=a^{1/2^3}\)。這正是 \(112\) 單1 的核心概念。]]）。"]},
                {"label": r"解指數方程", "lines": [
        ...
        {
            "num": "核心概念 2", "id": "kp2", "nav": "對數的定義與運算", "freq": "★★☆",
        ...
        {
            "num": "核心概念 3", "id": "kp3", "nav": "常用對數（科學記號、位數）", "freq": "★★☆",
            ...
            "prereq": [
                r"**常用對數**：\(\log\) 不寫底 ＝ 底 \(10\)（核心概念 2）",
                r"**次方化乘** \(\log N^k=k\log N\)（核心概念 2）",
        ...
        {
            "num": "核心概念 4", "id": "kp4", "nav": "指數對數函數圖形", "freq": "★★★ 必考",
            "title": r"指數與對數函數的圖形",
            ...
            "prereq": [
                r"**指對互換**：\(a^x=N\Leftrightarrow\log_a N=x\)（核心概念 2）",
        ...
        {
            "num": "核心概念 5", "id": "kp5", "nav": "應用模型與數列結合", "freq": "★★★ 必考",
            ...
            "prereq": [
                r"**指數律** \(\left(\dfrac12\right)^{t/T}\) 的計算（核心概念 1）",
                r"**等比／等差數列**的公比、公差（數列單元）",
                r"**取 \(\log\)**：\(\log y=\log k+x\log a\)（核心概念 2）",
                r"**三點共線 \(\Leftrightarrow\) 兩兩斜率相等**（直線與圓單元 核心概念 1）——\(115\) 選15「\(x\) 等距 \(\Rightarrow\) 共線⟺\(y\) 成等差」的依據",
            ],
            "points": [
            ...
            "questions": [
                ...
                {"tag": "111 數A · 單選 4", "level": "★★☆（中）", "core": "等差數列 ＋ 對數律：等比中項",
                 "body": r"設等差數列 \(\langle a_n\rangle\) 之首項 \(a_1\) 與公差 \(d\) 皆為正數，且 \(\log a_1,\log a_3,\log a_6\) 依序也成等差數列。試選出數列 \(\log a_1,\log a_3,\log a_6\) 的公差。",
                 "options": [
                     r"\(\log d\)",
                     r"\(\log\dfrac23\)",
                     r"\(\log\dfrac32\)",
                     r"\(\log 2d\)",
                     r"\(\log 3d\)"],
                 "solution": {"brief": r"(3) \(\log\dfrac32\)",
                              "steps": [
                                  r"\(\log a_1,\log a_3,\log a_6\) 成等差 \(\Leftrightarrow 2\log a_3=\log a_1+\log a_6\Leftrightarrow a_3^2=a_1a_6\)（\(a_3\) 為 \(a_1,a_6\) 的等比中項）。",
                                  r"\((a_1+2d)^2=a_1(a_1+5d)\Rightarrow a_1^2+4a_1d+4d^2=a_1^2+5a_1d\Rightarrow4d^2=a_1d\Rightarrow a_1=4d\)（\(d>0\)）。",
                                  r"故 \(a_1=4d,\ a_3=6d,\ a_6=9d\)，公差 \(=\log a_3-\log a_1=\log\dfrac{6d}{4d}=\log\dfrac32\)，選 (3)。"]}},
                {"tag": "112 數A · 多選 9", "level": "★★★（難）", "core": "等比數列取對數 ＋ 交錯和不等式",
                 "body": r"設 \(a_1,a_2,a_3,\dots,a_n\) 是首項為 \(3\)、公比為 \(3\sqrt3\) 的等比數列。試選出滿足不等式 \(\log_3a_1-\log_3a_2+\log_3a_3-\log_3a_4+\cdots+(-1)^{n+1}\log_3a_n>18\) 的項數 \(n\) 之可能選項。",
                 "options": [r"\(23\)", r"\(24\)", r"\(25\)", r"\(26\)", r"\(27\)"],
                 "solution": {"brief": r"(3)(5)",
                              "steps": [
                                  r"令 \(b_k=\log_3a_k\)。\(a_k=3\cdot(3\sqrt3)^{k-1}=3^{1+\frac32(k-1)}\)，故 \(b_k=\dfrac{3k-1}{2}\)（等差、公差 \(\tfrac32\)）。",
                                  r"交錯和 \(S=b_1-b_2+b_3-\cdots+(-1)^{n+1}b_n\)。\(n\) 偶：每對 \(b_{奇}-b_{偶}=-\tfrac32\)，\(S=-\dfrac{3n}{4}<0\)，不合。",
                                  r"\(n\) 奇：\(S=-\dfrac32\cdot\dfrac{n-1}{2}+b_n=\dfrac{3n+1}{4}\)。要 \(S>18\Rightarrow3n+1>72\Rightarrow n>\dfrac{71}{3}\approx23.7\)，且 \(n\) 為奇數 \(\Rightarrow n\ge25\)。",
                                  r"選項中奇數且 \(\ge25\)：\(25\)（\(S=19\)）、\(27\)（\(S=20.5\)）。故選 (3)(5)。"]}},
                {"tag": "115 數A · 選填 15", "level": "★★★（難）", "core": "等差數列 ＋ 對數律：三點共線",
                 "body": r"已知三正數 \(a,b,c\) 成一等差數列，其中 \(a<b<c\)，且坐標平面上三點 \((a,\log 3a)、(b,\log 4b)、(c,\log 6c)\) 在同一直線上，則 \(\dfrac ba\) 之值為（最簡分數）。",
                 "solution": {"brief": r"\(\dfrac32\)",
                              "steps": [
                                  r"三點 \(x\) 坐標 \(a,b,c\) 成等差（等距），故共線 \(\Leftrightarrow y\) 坐標也成等差：\(2\log(4b)=\log(3a)+\log(6c)\Rightarrow(4b)^2=18ac\Rightarrow8b^2=9ac\)。",
                                  r"由等差 \(c=2b-a\) 代入：\(8b^2=9a(2b-a)\Rightarrow8b^2-18ab+9a^2=0\)。令 \(t=\dfrac ba\)：\(8t^2-18t+9=0\Rightarrow t=\dfrac{18\pm6}{16}=\dfrac32\) 或 \(\dfrac34\)。",
                                  r"因 \(a<b\Rightarrow t>1\)，取 \(t=\dfrac32\)。（驗：\(a,\tfrac32a,2a\) 對應 \(\log3a,\log6a,\log12a\)，差皆 \(\log2\)，確實共線。）"]}},
            ],
            "selfcheck": {
